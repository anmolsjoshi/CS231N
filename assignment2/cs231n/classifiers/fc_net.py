...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            self.params['W%i' % i] = weight_scale*np.random.randn(in_dim[i - 1], out_dim[i - 1])
            self.params['b%i' % i] = weight_scale * np.zeros(out_dim[i - 1])
        ...
        if self.use_batchnorm:
            ...
            for i in range(1, self.num_layers):
                self.params['gamma%i' % i] = np.ones(out_dim[i - 1])
                self.params['beta%i' % i] = np.zeros(out_dim[i - 1])
    ...
    def loss(self, X, y=None):
        ...
        cache_affine, cache_relu, cache_bn, cache_dropout = [], [], [], []
        ...
        for i in range(self.num_layers):
            ...
            if self.use_batchnorm and i != self.num_layers - 1:
                out, cache = batchnorm_forward(out, self.params['gamma'+str(i+1)], self.params['gamma'+str(i+1)], self.bn_params[i])
        ...
        for i in range(self.num_layers - 1, -1, -1):
            ...
            if i == self.num_layers - 1:
                dout, grads[W], grads[b] = affine_backward(dout, cache_affine[i])
            ...
            if self.use_batchnorm:
                gamma = 'gamma' + str(i+1)
                beta = 'beta' + str(i+1)
                dout, grads[gamma], grads[beta] = batchnorm_backward(dout, cache_bn[i])
